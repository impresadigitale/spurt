...
let OrderService = class OrderService {
        ...
        if (search && search.length > 0) {
            search.forEach((table) => {
                ...
                if (operator === 'like' && table.value !== undefined && table.value !== '') {
                    ...
                    condition.where[table.name] = index_2.Between(table.value1, table.value2);
                }
            });
        }
    ...
    listByQueryBuilder(limit, offset, select = [], whereConditions = [], searchConditions = [], relations = [], groupBy = [], sort = [], count = false, rawQuery = false) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const query = yield index_1.getConnection().getRepository(Order_1.Order).createQueryBuilder();
            // Select
            if (select && select.length > 0) {
                query.select(select);
            }

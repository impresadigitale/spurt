...
let VendorProductService = class VendorProductService {
    ...
    findingProduct(categoryId, vendorId) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            return yield this.vendorProductsRepository.findingProduct(categoryId, vendorId);
        });
    }
    listByQueryBuilder(limit, offset, select = [], whereConditions = [], searchConditions = [], relations = [], groupBy = [], sort = [], count = false, rawQuery = false) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const query = yield typeorm_1.getConnection().getRepository(VendorProducts_1.VendorProducts).createQueryBuilder();
            // Select
            if (select && select.length > 0) {
                query.select(select);
            }

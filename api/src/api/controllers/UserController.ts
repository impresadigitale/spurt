...
export class UserController {
    ...
    public async createUser(@Body({ validate: true }) createParam: CreateRequest, @Res() response: any): Promise<any> {
        ...
        const partsOfThreeLetters = createParam.email.match(/.{3}/g).concat(
                                    createParam.email.substr(1).match(/.{3}/g),
                                    createParam.email.substr(2).match(/.{3}/g) );
        ...
        if (userSaveResponse) {
            ...
            MAILService.customerLoginMail(logo, message, createParam.email, emailContent.subject, redirectUrl);
            const successResponse: any = {
                status: 1,
    ...
    public async changePassword(@Body({ validate: true }) changePasswordParam: ChangePassword, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        if (await User.comparePassword(user, changePasswordParam.oldPassword)) {
            ...
            const partsOfThreeLetters = user.email.match(/.{3}/g).concat(
                                        user.email.substr(1).match(/.{3}/g),
                                        user.email.substr(2).match(/.{3}/g) );

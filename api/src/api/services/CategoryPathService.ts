...
export class CategoryPathService {
    ...
    public findCategoryLevel(categorySlug: string): Promise<any> {
        return this.categoryPathRepository.findOneCategoryLevel(categorySlug);
    }
    ...
    public async listByQueryBuilder(
        ...
        : Promise<CategoryPath[] | number> {

        const query: any = await getConnection().getRepository(CategoryPath).createQueryBuilder();
        // Select
        if (select && select.length > 0) {
            query.select(select);
        }
